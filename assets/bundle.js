(function () {
  ...
  class BundleBlock extends HTMLElement {
    ...
    getMax() {
      ...
      return count + singleGroups.size;
    }
  ...
  class CardProductBundle extends HTMLElement {
    ...
    _soldOutHTML() {
      ...
      if (defined) this.bundleBlock?.updateFormAddToCartText();
    ...
    _addHTML() {
      const available = !this._data.variantAvailable;

      return `<div class="card-bundle__controls" slot="controls">
      <button data-action="add" class="btn btn--primary card-bundle__btn--add" type="button" ${available ? 'disabled':''}>ADD</button>
    </div>`;
    }
